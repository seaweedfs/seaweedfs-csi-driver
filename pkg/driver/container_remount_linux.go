...
func remountViaSetns(containerPID int, containerMountPath, stagingPath string, readOnly bool) error {
	...
	// open_tree(OPEN_TREE_CLONE) gives us an fd-anchored detached clone
	// of the mount tree. Plain unix.Mount(stagingPath, ...) after setns
	// fails because /var/lib/kubelet/... does not exist inside the
	// container's image filesystem; /proc/self/fd/N also fails because
	// the kernel re-walks the original path in the new namespace.
	// Linux 5.2+.
